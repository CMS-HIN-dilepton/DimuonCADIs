                                 ...
                                 )
{
  ...
  if (incJpsi && !incPsi2S) {
    themodel = new RooAddPdf(Form("%s_Tot_%s", pdfName.c_str(), (isPbp?"Pbp":"PP")), Form("%s_Tot_%s", pdfName.c_str(), (isPbp?"Pbp":"PP")), 
                             RooArgList( 
                                        *ws.pdf(Form("%sTot_JpsiNoPR_%s", pdfName.c_str(), (isPbp?"Pbp":"PP")))
                                         )
                             );
  }
  if (!incJpsi && incPsi2S) {
    themodel = new RooAddPdf(Form("%s_Tot_%s", pdfName.c_str(), (isPbp?"Pbp":"PP")), Form("%s_Tot_%s", pdfName.c_str(), (isPbp?"Pbp":"PP")), 
                             RooArgList( 
                                        *ws.pdf(Form("%sTot_Psi2SNoPR_%s", pdfName.c_str(), (isPbp?"Pbp":"PP")))
                                         )
                             );
  }
  if (incJpsi && incPsi2S) {
  ...
  switch(model) 
    ...
    case (CtauModel::SingleGaussianResolution):
 	...
 	cout << Form("[ERROR] Initial parameters where not found for Single Gaussian Ctau Truth Resolution Model in %s", (isPbp?"Pbp":"PP")) << endl; return false; 
      ...
      //ws.factory(Form("GaussModel::%s(%s, %s, %s)", Form("pdfCTAUTRUERES_%s_%s", object.c_str(), (isPbp?"Pbp":"PP")), "ctauTrue", 
...
bool addSignalCtauTrueModel(RooWorkspace& ws, string object, CtauModel model, map<string,string> parIni, bool isPbp) 
{
  ...
  switch(model) 
    ...
    case (CtauModel::Delta):
      ...
      ws.factory(Form("SUM::%s(%s)", Form("pdfCTAUTRUE_%s_%s", object.c_str(), (isPbp?"Pbp":"PP")),
 		      Form("pdfCTAUTRUERES_%s_%s", object.c_str(), (isPbp?"Pbp":"PP"))
 		      ));
...
void setCtauTrueDefaultParameters(map<string, string> &parIni, bool isPbp, double numEntries)
{
 ...
 // Resolution Ctau Truth Model
  if (parIni.count(Form("ctauMC_JpsiNoPR_%s", (isPbp?"Pbp":"PP")))==0 || parIni[Form("ctauMC_JpsiNoPR_%s", (isPbp?"Pbp":"PP"))]=="") {
    //parIni[Form("ctauMC_JpsiNoPR_%s", (isPbp?"Pbp":"PP"))] = Form("%s[%.4f,%.4f,%.4f]", Form("ctauMC_JpsiNoPR_%s", (isPbp?"Pbp":"PP")), 0.0, -100.0, 100.0);
  ...
  if (parIni.count(Form("ctauMC_Psi2SNoPR_%s", (isPbp?"Pbp":"PP")))==0 || parIni[Form("ctauMC_Psi2SNoPR_%s", (isPbp?"Pbp":"PP"))]=="") { 
    //parIni[Form("ctauMC_Psi2SNoPR_%s", (isPbp?"Pbp":"PP"))] = Form("%s[%.4f,%.4f,%.4f]", Form("ctauMC_Psi2SNoPR_%s", (isPbp?"Pbp":"PP")), 0.0, -100.0, 100.0);
