                             ...
                             )
{
  ...
  if (incJpsi && incPsi2S && incBkg) {
    themodel = new RooAddPdf(pdfName.c_str(), pdfName.c_str(), 
                      RooArgList( 
                                 *ws.pdf(Form("pdfMASSTot_Jpsi_%s", (isPbp?"Pbp":"PP"))), 
                                 *ws.pdf(Form("pdfMASSTot_Psi2S_%s", (isPbp?"Pbp":"PP"))), 
                                 *ws.pdf(Form("pdfMASSTot_Bkg_%s", (isPbp?"Pbp":"PP"))) 
                                  )
                      );
  }
  if (incJpsi && incPsi2S && !incBkg) {
...
void setMassDefaultParameters(map<string, string> &parIni, bool isPbp, double numEntries)
{
  ...
  if (parIni.count(Form("N_Jpsi_%s", (isPbp?"Pbp":"PP")))==0 || parIni[Form("N_Jpsi_%s", (isPbp?"Pbp":"PP"))]=="") { 
    parIni[Form("N_Jpsi_%s", (isPbp?"Pbp":"PP"))]  = Form("%s[%.10f,%.10f,%.10f]", Form("N_Jpsi_%s", (isPbp?"Pbp":"PP")), numEntries, 0.0, 2.0*numEntries);
  }
  if (parIni.count(Form("N_Psi2S_%s", (isPbp?"Pbp":"PP")))==0 || parIni[Form("N_Psi2S_%s", (isPbp?"Pbp":"PP"))]=="") { 
    parIni[Form("N_Psi2S_%s", (isPbp?"Pbp":"PP"))]  = Form("%s[%.10f,%.10f,%.10f]", Form("N_Psi2S_%s", (isPbp?"Pbp":"PP")), numEntries, 0.0, 2.0*numEntries);
  }
  if (parIni.count(Form("N_Bkg_%s", (isPbp?"Pbp":"PP")))==0 || parIni[Form("N_Bkg_%s", (isPbp?"Pbp":"PP"))]=="") { 
    parIni[Form("N_Bkg_%s", (isPbp?"Pbp":"PP"))]  = Form("%s[%.10f,%.10f,%.10f]", Form("N_Bkg_%s", (isPbp?"Pbp":"PP")), numEntries, 0.0, 2.0*numEntries);
  }
